...
class BobPaymentServices
{
    ...
    public function updatedTransactionInHostedSessionToPay($orderid, $trans, $suyooler = null, $receiverPhone = null, $senderPhone = null, $senderInitials = null, $merchantName = null, $simulation = null)
    {
        ...
        if (isset($content['error'])) {
            ...
            $title = "Please Try Again";
            $description = "An error has occurred with your card payment. <br>Please try again later or use another payment method.";
            $imgsrc = "build/images/Loto/error.png";
            $button = "Try Again";
            $parameters = [
                'title' => $title,
        ...
        $attempts->setSuyoolUserId($suyooler)
            ...
            ->setErrorMessage(@$content['response']['acquirerMessage']);
        $this->mr->persist($attempts);
        $this->mr->flush();
        ...
        if ($content['order']['status'] == 'CAPTURED') {
            if (is_null($suyooler)) {
                ...
                if ($attemptsPerCardSum[0] > 1500.000) {
                    ...
                    $emailMessageUpTo5Thousands .= "</ul><br><br>Please initiate the necessary protocol for further investigation and action.<br><a href='https://suyool.com'>Suyool.com</a>";
                    if ($_ENV['APP_ENV'] == 'dev' || $_ENV['APP_ENV'] == 'test') {
                        $this->suyoolServices->sendDotNetEmail('[Alert] Suspected Fraudulent Topup Transaction', 'anthony.saliba@elbarid.com', $emailMessageUpTo5Thousands, "", "", "no-reply@suyool.com", "no-reply", 1, 0);
                    } else {
                        $this->suyoolServices->sendDotNetEmail('[Alert] Suspected Fraudulent Topup Transaction', 'web@suyool.com,it@suyool.com,arz@elbarid.com', $emailMessageUpTo5Thousands, "", "", "no-reply@suyool.com", "no-reply", 1, 0);
                    }
                    $this->logger->info('Send email 2');
                }
                $this->logger->info(json_encode($attemptsPerCard));
                if ($attemptsPerCard[0] >= 2) {
                    ...
                    $emailMessageUpTo2Times .= "</ul><br><br>Please initiate the necessary protocol for further investigation and action.<br><a href='https://suyool.com'>Suyool.com</a>";
                    if ($_ENV['APP_ENV'] == 'dev' || $_ENV['APP_ENV'] == 'test') {
                        $this->suyoolServices->sendDotNetEmail('[Alert] Suspected Fraudulent Topup Transaction', 'anthony.saliba@elbarid.com', $emailMessageUpTo2Times, "", "", "no-reply@suyool.com", "no-reply", 1, 0);
                    } else {
                        $this->suyoolServices->sendDotNetEmail('[Alert] Suspected Fraudulent Topup Transaction', 'web@suyool.com,it@suyool.com,arz@elbarid.com', $emailMessageUpTo2Times, "", "", "no-reply@suyool.com", "no-reply", 1, 0);
                    }
                    $this->logger->info('Send email');
                }
            ...
            if (is_null($suyooler)) {
                ...
                if (isset($content['response']['acquirerMessage']) && ($content['response']['acquirerMessage'] == "Transaction not permitted to cardholder" || $content['response']['acquirerMessage'] == "Do not honor")) {
                    $title = "Payment Unsuccessfull";
                    $description = "An error has occurred with your card payment. <br>Please call your issued bank for more information or try again later.";
                } else {
                    $title = "Please Try Again";
                    $description = "An error has occurred with your card payment. <br>Please try again later or use another payment method.";
                }
    ...
    public function updatedTransactionInHostedSessionToPayTopup($orderid, $trans, $suyooler = null, $receiverPhone = null, $senderPhone = null)
    {
        ...
        // $content['response']['acquirerMessage'] = "Do not honor";
        $attempts->setSuyoolUserId($suyooler)
            ->setReceiverPhone($receiverPhone)
            ->setSenderPhone($senderPhone)
            ->setResponse(json_encode($content))
        ...
        if ($content['order']['status'] == 'CAPTURED') {
            ...
            if ($attemptsPerCardSum[0] > 1500.000) {
                ...
                $emailMessageUpTo5Thousands .= "</ul><br><br>Please initiate the necessary protocol for further investigation and action.<br><a href='https://suyool.com'>Suyool.com</a>";
                if ($_ENV['APP_ENV'] == 'dev' || $_ENV['APP_ENV'] == 'test') {
                    $this->suyoolServices->sendDotNetEmail('[Alert] Suspected Fraudulent Topup Transaction', 'anthony.saliba@elbarid.com', $emailMessageUpTo5Thousands, "", "", "suyool@suyool.com", "Suyool", 1, 0);
                } else {
                    $this->suyoolServices->sendDotNetEmail('[Alert] Suspected Fraudulent Topup Transaction', 'web@suyool.com,it@suyool.com,arz@elbarid.com', $emailMessageUpTo5Thousands, "", "", "suyool@suyool.com", "Suyool", 1, 0);
                }
                $this->logger->info('Send email 2');
                $this->logger->info('Send email 2');
            }
            ...
            if (isset($content['response']['acquirerMessage']) && ($content['response']['acquirerMessage'] == "Transaction not permitted to cardholder" || $content['response']['acquirerMessage'] == "Do not honor")) {
                $imgsrc = "build/images/Loto/error.png";
                $title = "Payment Unsuccessfull";
                $description = "An error has occurred with your top up. <br>Please call your issued bank for more information or try again later.";
                $button = "Try Again";
                $parameters = [
                    'title' => $title,
                    'imgsrc' => $imgsrc,
