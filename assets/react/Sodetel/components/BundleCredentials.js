...
function BundleCredentials({
  ...
  setBackLink("Default");
  setHeaderTitle(
    `Re-charge ${capitalizeFirstLetters(activeButton?.bundle)} Package`
  );
  ...
  const handleInputChange = (event) => {
    ...
    if (credential.type === "phone") {
      setCredential({
        ...credential,
        [credential.name]: formatMobileNumber(value),
      });
    } else {
      ...
      {credentialsArray.length > 1 && showCredentials ? (
        <div id="Default_Sodetel" className="py-0">
          <div className="MainTitle">
            Re-charge your {capitalizeFirstLetters(activeButton.bundle)} Package
            using your:
          </div>
            ...
            >
              ...
              <div className="Text">
                ...
                <div className="description">
                  Settle your Sodetel bill quickly and securely using your{" "}
                  {credential.label}
                </div>
              </div>
            </div>
        ...
        <>
          <div className="mainTitle">
            Enter your {credential.label} to recharge
          </div>
          {credential.type === "phone" ? (
            ...
            <div className="MobileNbContainer mt-3">
              <input
                type="text"
                className={
                  getSpinnerLoader ? "nbInputHide w-100" : "nbInput w-100"
                }
                placeholder={credential.label}
                value={credential.mobileNumber}
                onChange={handleInputChange}
          ...
          {!showCredentials &&
            ...
            credentialsArray.map((credentialObj, index) => {
              if (credentialObj.name !== credential.name) {
                return (
                  <button
                    className="credential-link my-3"
                    onClick={() => {
