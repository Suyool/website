        ...
        if (data.success == "Invalid Email") {
          ...
          if (data.success) {
            emailStatus.textContent =
              "You will be the first one to know once the Suyool app is launched.";
...
$(".submitTextDownloadAppBtn").on("click", function (e) {
  ...
  var mobileNumber = $(this)
    .closest(".input-main-cont")
    .find(".mobileTextDownloadApp")
    .val();
  // Send an AJAX request to the server
  $.ajax({
    url: "/invitationCard/submitInvitationCard",
    method: "POST",
    data: { mobileNumber: mobileNumber, code: $(this).attr("code") },
    success: function (response) {
      // Show the response message in the modal
      $("#invitationModal .modal-body").append(
        "<p>" + response.RespDesc + "</p>"
      );
...
$(document).ready(function () {
  ...
  $(".mobileTextDownloadApp").on("input", function (event) {
    ...
    if ($(this).val().length > 6) {
      ...
      $(mainDivision + " .submitTextDownloadAppBtn").attr(
        "disabled",
        "disabled"
      );
    }
...
function resubscribe(uniqueCode, flag) {
  jQuery.ajax({
    type: "GET",
    url:
      "/unsubscribeMarketing/resubscribe?uniqueCode=" +
      uniqueCode +
      "&flag=" +
      flag,
    dataType: "json",
